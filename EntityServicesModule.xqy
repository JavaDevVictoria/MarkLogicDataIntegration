...
(:
 ...
 The source entity type document was procycling-0.0.1
 ...
 Generated at timestamp: 2020-03-12T21:23:24.289402Z
 Persisted by AUTHOR
 Date: DATE
 :)

module namespace procycling
    = 'http://procycling.com/entity-types#procycling-0.0.1';
...
(:~
 ...
 :)
declare function procycling:extract-instance-Product(
    $source as node()?
) as map:map
{
    let $source-node := procycling:init-source($source, 'Product')

    let $instance := json:object()
    (: If desired, add the original source document as an attachment. :)
        =>procycling:add-attachments($source-node, $source)
    ...
    $instance
    =>   map:with('id',                     xs:string($source-node/prodId))
    =>   map:with('name',                   xs:string($source-node/name))
    =>   map:with('price',                  xs:double($source-node/price))
    =>es:optional('weightKg',               xs:double($source-node/weightKg))
    =>es:optional('weightLb',               xs:double($source-node/weightLb))
    =>es:optional('material',               xs:string($source-node/madeOf))
    =>es:optional('gender',                 xs:string($source-node/gender))
    =>es:optional('color',                  xs:string($source-node/color))
    =>es:optional('description',            xs:string($source-node/description))
    =>es:optional('speeds',                 xs:decimal($source-node/speeds))
    (: The following property is a local reference.  :)
    =>es:optional('recommend',              es:extract-array($source-node/recommend, procycling:extract-instance-Product#1))
};
...
(:~
 ...
 :)
declare function procycling:instance-to-canonical-xml(
            ...
            return
                ...
                case json:object+
                    return
                        ...
                        return element { $key } { procycling:instance-to-canonical-xml($prop) }
                    ...
                    return
                        ...
                        return
                            ...
                            then element { $key } {
                                ...
                                procycling:instance-to-canonical-xml($val) }
                            else element { $key } {
                                attribute datatype { 'array' },
                                $val }
...
(:
 ...
 :)
declare function procycling:instance-to-envelope(
    $entity-instance as map:map
) as document-node()
{
    document {
        element es:envelope {
            element es:instance {
                ...
                procycling:instance-to-canonical-xml($entity-instance)
            },
...
declare private function procycling:init-source(
    $source as node()*,
...
declare private function procycling:add-attachments(
